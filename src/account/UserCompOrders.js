...
const UserCompOrders = () => {
    ...
    const getOrders = async () => {
        ...
        setTimeout(() => {
            fetch(process.env.REACT_APP_API + "/Orders/GetCompletedOrderWithDetailByUserId?id=" + user.id, {
                method: 'GET',
                headers: {
        ...
        <div>
            ...
            <Spin spinning={order.loading}>
                {order.data.length > 0 ? order.data.map((item, index) => (
                    <div key={index} className='mt-8 rounded bg-white border-gray-300 border-opacity-100 border divide-y'>
                        ...
                        {item.orderDetail.map((detail, i) => (
                            <Row key={i} className='py-4 px-8 flex items-center'>
                                <Col span={6}>
                                    <p className='text-xs font-semibold text-green-500'>{item.status}</p>
                                    <p className='text-xs'>{dateFormat(item.updatedDate)} tarihinde teslim edilmiştir</p>
                                </Col>
                    ...
                    :
                    <div>{order.loading === false && "Tamamlanan Sipariş Bulunamadı."}</div>}
            </Spin>
        </div>
    )
}

export default UserCompOrders;
